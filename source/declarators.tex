...
(\ref{class.ctor}), copy or move constructor, or copy or move
assignment operator (\ref{class.copy}) is so declared.
...
\begin{itemize}
\item
If T1 is a class type, user-defined conversions are considered using the
rules for copy-initialization of an object of type 
``\nonterminal{cv1} \tcode{T1}''
by user-defined conversion (\ref{dcl.init}, \ref{over.match.copy});
...
\item
If T1 is a non-class type, 
a temporary of type ``\nonterminal{cv1} \tcode{T1}'' is created and
copy-initialized from the initializer expression.
