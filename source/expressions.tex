...
(Clause~\ref{expr}),
in a template-argument, in an alias-declaration, in a typedef
declaration, or in the declaration of a function or function
...
\exitnote
\enternote A closure object behaves like a function
...
\tcode{this}) is said to be \defn{explicitly captured} if it is found by this
process.

\pnum
For every \grammarterm{init-capture} a non-static data member named by the
\grammarterm{identifier} of the \grammarterm{init-capture} is declared in the closure type.
This member is not a bit-field and not \tcode{mutable}.
The type of that member corresponds to the type of a hypothetical variable
declaration of
the form ``\tcode{auto} \grammarterm{init-capture} \tcode{;}'', except that the
variable name (i.e., the \grammarterm{identifier} of the \grammarterm{init-capture}) is
...
\exitnote
No entity is captured by an \grammarterm{init-capture}.  Within the
\grammarterm{lambda-expression}{s} \grammarterm{lambda-declarator} and
\grammarterm{compound-statement},
the \grammarterm{identifier} in the \grammarterm{init-capture} hides any declaration
of the same name in scopes enclosing the \grammarterm{lambda-expression}.
\enterexample
\begin{codeblock}
int x = 4;
...
\pnum
A glvalue, class prvalue, or array prvalue of
type ``\cvqual{cv1} \tcode{T1}'' can be cast to type ``rvalue
...
well-formed, for some invented variable
...
storage for a \grammarterm{new-expression} \tcode{e2} if the lifetime of the
object allocated by \tcode{e1} strictly contains the lifetime of the object
allocated by \tcode{e2}, \tcode{e1} and \tcode{e2} would invoke the same
replaceable global allocation function, and, for a throwing allocation
function, exceptions in \tcode{e1} and \tcode{e2} would be first caught in the
same handler.
...
be deleted was not omitted~(\ref{expr.new}), the
